 ...
 *
 * @returns {Promise<QuestionResponse | AnswerResponse | null>} - Promise that resolves to the
 *          populated question or answer, or null if not found, or error message.
 */
export const populateDocument = async (
  ...
  try {
    ...
    if (type === 'question') {
      result = await QuestionModel.findOne({ _id: id }).populate('comments');
    } else if (type === 'answer') {
      result = await AnswerModel.findOne({ _id: id }).populate('comments');
    }

    return result;
  } catch (error) {
    return { error: 'Error when fetching and populating a question' };
  }
};
...
export const getTags = async (tags: Tag[]): Promise<Tag[]> => {
  try {
    // Remove duplicate tags based on their names using a Set
    const uniqueTagNames = new Set(tags.map(tag => tag.name));
    const uniqueTags = Array.from(uniqueTagNames).map(name => tags.find(tag => tag.name === name)!);

    // Use Promise.all to check for existing tags and add only non-existing ones
    const addedTags = await Promise.all(
      uniqueTags.map(async tag => {
        const existingTag = await TagModel.findOne({ name: tag.name });

        if (existingTag) {
          return existingTag; // Return existing tag if found
        }
        const addedTag = await addTag(tag); // Add new tag if not found
        if (addedTag) {
          return addedTag;
        }
...
export const addComment = async (
  ...
  type: 'question' | 'answer',
  comment: Comment,
): Promise<Question | Answer | { error: string }> => {
  try {
    // Validate input
    if (!id || (type !== 'question' && type !== 'answer') || !comment) {
      throw new Error('Invalid request data');
    }

    // Save the comment and get its ID
    const savedComment = await saveComment(comment);
    if ('error' in savedComment) {
      throw new Error(savedComment.error);
    }

    // Initialize the variable for updated document
    let updatedDocument: Question | Answer | null = null;
    if (type === 'question') {
      updatedDocument = await QuestionModel.findOneAndUpdate(
        { _id: id },
        { $push: { comments: savedComment._id } },
        { new: true },
      ).populate('comments');
    } else if (type === 'answer') {
      updatedDocument = await AnswerModel.findOneAndUpdate(
        { _id: id },
        { $push: { comments: savedComment._id } },
        { new: true },
      ).populate('comments');
    ...
    if (!updatedDocument) {
      throw new Error(`Failed to add comment to the ${type}`);
    }
